...
| alfama-3 | Propostas: list + detail scoped by `(workspace_id, project_id, id)` | `ai-memory-web/src/routes/painel_proposals.rs` — listing (`ReaderPool::list_auto_improve_proposals`) filters by `(workspace_id, project_id)`; detail (`ReaderPool::auto_improve_proposal_detail`, `ai-memory-store/src/reader.rs`) resolves `id` only within that same scope (`WHERE id = ?1 AND workspace_id = ?2 AND project_id = ?3`), so a proposal id belonging to another project never surfaces, not even by direct id — there is no per-id web route, so the boundary is exercised at the store call the web makes | `ai-memory-web/tests/suite/routes.rs` `propostas_de_outro_projeto_nao_vazam_na_listagem_nem_no_detalhe` (adversarial: project B's pending proposal absent from project A's `?status=pending` listing and from `auto_improve_proposal_detail(ws, proj_A, id_of_B)`; control: the same proposal appears in project B's own listing), `propostas_de_projeto_inexistente_ou_de_outro_workspace_responde_404` (route-level: unknown project and a project under a foreign workspace both 404; control: the same project under its real workspace 200s), `propostas_de_update_com_pagina_alvo_mudada_mostra_conflito`, `propostas_escapa_titulo_e_justificativa_maliciosos`. Bitten and confirmed by hand: dropping the `project_id` filter from the detail query's `WHERE` clause made the adversarial test fail; reverted immediately after confirming | STRONG |
| alfama-4 | Entre projetos: cross-project aggregation stays owner-scoped for handoffs, and rule grouping never crosses workspaces; messages are project-shared by design (invariant #16), not actor-scoped | Rules: `ai-memory-store/src/painel.rs` `ReaderPool::rules_across_projects(ws)` reads latest `rule`-kind pages (invariant #16, shared, no owner filter, by design) across every project of the *given* workspace only — never across workspaces; embeddings are attached only when their `(provider, model, dim)` is that workspace's majority triple (invariant #8), else `None` with a normalized-title fallback in the pure `group_rules`. Handoffs/messages: `ai-memory-web/src/routes/painel_cross_project.rs` — handoffs go through the SAME `OwnerFilter` (`owner_filter_for`, made `pub(crate)` and reused from `api.rs`, not duplicated) and the SAME handoff-body redaction (`serves_handoff_body`, reused); pending messages are listed per-project via `ReaderPool::list_messages(ws, proj, MessageBox::Inbox, ..)`, which has no owner/actor parameter at all — `AgentMessage::origin.from_owner_user` is documented "Attribution/provenance only — never a read filter" (`ai-memory-core/src/message.rs`), so messages are shown to every caller regardless of identity, same as any other shared page | Rules: `ai-memory-store/tests/suite/painel.rs` `similar_rules_across_projects_group_together`, `no_vector_groups_by_normalized_title`, `same_project_pair_alone_does_not_form_a_group`, `zero_norm_vectors_never_link`, `vectors_of_different_dims_are_never_compared`, `rules_across_projects_ignores_vectors_of_a_non_majority_triple`, `non_majority_model_vectors_fall_back_to_title_matching`, `malformed_embedding_blob_becomes_none_without_panicking`, `embedding_length_mismatched_with_dim_becomes_none`; adversarial: `rules_from_another_workspace_never_appear_or_group` (workspace A never sees workspace B's equivalent rule, and its own single-project result cannot form a cross-project group; control: within workspace B alone, the same rule repeated in a second project of that workspace does group). Bitten and confirmed by hand: dropping the `ws` filter from `rules_across_projects`'s two queries made the adversarial test fail (workspace A saw both rules and grouped them); reverted immediately after confirming. Web screen (`ai-memory-web/tests/suite/routes.rs`): `entre_projetos_agrupa_regras_equivalentes_de_dois_projetos`, `entre_projetos_lista_handoff_aberto_mas_nao_o_aceito`, `entre_projetos_lista_mensagem_pendente_origem_destino`, `entre_projetos_escapa_conteudo_malicioso`; adversarial: `entre_projetos_nao_vaza_handoff_de_outro_dono` (another operator's open handoff absent for an authenticated actor; control: the actor's own appears) — bitten by hand: replacing `owner_filter_for(actor)` with `OwnerFilter::Any` in the handler made it fail (both handoffs appeared), reverted immediately; `entre_projetos_oculta_resumo_quando_o_ator_nao_pode_ler_o_corpo` (a handoff summary this caller may not read per `serves_handoff_body` never reaches the HTML); `entre_projetos_mensagem_pendente_nao_vaza_para_projeto_sem_mensagem` (messages have no actor to test against, so the adversarial half checks project scoping instead: a project with nothing pending does not appear on the page at all) — bitten by hand: swapping `MessageBox::Inbox` for `MessageBox::Outbox` in the handler made the message's control test fail, reverted immediately | STRONG |
