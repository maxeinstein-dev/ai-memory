...
| alfama-2 | Linha do tempo scoped to `(workspace_id, project_id)` | `ai-memory-store/src/painel.rs` `ReaderPool::linha_do_tempo` filters sessions and `page_evidence` by `(workspace_id, project_id)` | a escrever — Tarefa 5 (Plano §Fase 2): sessão de outro projeto com id válido não aparece na linha do tempo de um projeto diferente | FUTURE |
